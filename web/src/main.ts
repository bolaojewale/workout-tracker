// App shell + a tiny hash router. Screens are scaffolds for now; each step of
// the roadmap (DESIGN.md §10) fills them in.
...
async function ping() {
  try {
    ...
    if (res.ok) console.info("API health:", await res.json());
  } catch {
    /* offline — fine */
  }
}
